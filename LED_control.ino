...
const int buttonPins[6] = {7, 6, 5, 4, 3, 2};
...
void setup() {
  ...
  // Setup Din pins for buttons.
  for (int i = 0; i < 6; i++) {
    pinMode(buttonStates[i], INPUT);
  }
...
void loop() {
  if (buttonStates[5] == 1) {        // Final stone inserted.
    buttonStates[5] = 1;
    if (timers[5] <= sparkDuration) {
      runPurpleSpark();
    }
    else if (timers[5] > sparkDuration && timers[5] <= 10000) {
      runPurpleEnergy();
    }
    // Strip 1-6 plays final combo.
    // Strip 1-6 loops idle pattern.
  }

  else if (buttonStates[4] == 1) {        // 5th stone inserted.
    buttonStates[4] = 1;
    if (timers[4] <= sparkDuration) {
      runPinkSpark();
    }
    else if (timers[4] > sparkDuration) {
      runPinkEnergy();
    }
    buttonStates[5] = digitalRead(buttonPins[5]);
  }

  else if (buttonStates[3] == 1) {         // 4th stone inserted.
    buttonStates[3] = 1;
    if (timers[3] <= sparkDuration) {
      runuGreenSpark();
    }
    else if (timers[3] > sparkDuration) {
      runGreenEnergy();
    }
    buttonStates[4] = digitalRead(buttonPins[4]);
  }

  else if (buttonStates[2] == 1) {       // 3rd stone inserted.
    buttonStates[2] = 1;
    if (timers[2] <= sparkDuration) {
      runRedSpark();
    }
    else if (timers[2] > sparkDuration) {
      runRedEnergy();
    }
    buttonStates[3] = digitalRead(buttonPins[3]);
  }
  
  else if (buttonStates[1] == 1) {        // 2nd stone inserted.
    buttonStates[1] = 1;
    if (timers[1] <= sparkDuration) {
      runBlueSpark();
    }
    else if (timers[1] > sparkDuration) {
      runBlueEnergy();
    }
    buttonStates[2] = digitalRead(buttonPins[2]);
  }
  
  else if (buttonStates[0] == 1) {         // 1st stone inserted.
    buttonStates[0] = 1;
    if (timers[0] <= sparkDuration) {
      runYellowSpark();
    }
    else if (timers[0] > sparkDuration) {
      runYellowEnergy();
    }
